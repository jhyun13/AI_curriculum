...
# 🔹 추천 결과 일관성 유지 (GPT 임베딩 기반 추천 시)
def set_seed(seed=42):
    np.random.seed(seed)
    random.seed(seed)

def get_args():
    parser = argparse.ArgumentParser(description="강의 추천 시스템(GPT 임베딩 기반)")
    ...
    parser.add_argument("--department_path", type=str, default="/root/ai_curri/content_based/GPT_EMB_new/data/depart_info.json", help="학과 소개글 임베딩 할 텍스트")
...
def main():
    ...
    recomm = Recommendation(db_handler, client)

    # 🔹 교과목 벡터 저장 (최초 1회 실행)
    if not os.path.exists("data/gpt_embeddings.npz"):
    ...
    output_dir = f'/root/ai_curri/content_based/GPT_EMB_new/output'
